...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None):
    ...
    # Set up rasterization configuration
    render_height = viewpoint_camera.semantic_feature.shape[1] 
    render_width = viewpoint_camera.semantic_feature.shape[2]
    tanfovx = math.tan(viewpoint_camera.FoVx * 0.5)
    tanfovy = math.tan(viewpoint_camera.FoVy * 0.5)
    raster_settings = GaussianRasterizationSettings(
        ...
        prefiltered=False,
        debug=pipe.debug,
        projmatrix_inv=viewpoint_camera.full_proj_transform.inverse(),
        degree_feature=pc.active_feature_sh_degree,
    )
    ...
    # Rasterize visible Gaussians to image, obtain their radii (on screen). 
    rendered_image, feature_map, alpha, depth, radii = rasterizer(
        means3D = means3D,
        means2D = means2D,
        shs = shs,
        colors_precomp = colors_precomp,

...
def prefetch_worker(camera_queue, prefetch_queue, stop_event):
    """Worker thread to prefetch camera data to GPU"""
    while not stop_event.is_set():
        try:
            cam = camera_queue.get(timeout=0.1)
            if cam is None:
                break
            # Prefetch to GPU
            cam.original_image = cam.original_image.cuda(non_blocking=True)
            cam.semantic_feature = cam.semantic_feature.cuda(non_blocking=True)
            prefetch_queue.put(cam)
        except:
            continue
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    gaussians = GaussianModel(dataset.sh_degree, feature_sh_degree=pipe.feature_sh_degree)
    scene = Scene(dataset, gaussians)
    
    # Setup prefetch queues and workers
    num_workers = 8
    camera_queue = Queue(maxsize=num_workers * 2)
    prefetch_queue = Queue(maxsize=num_workers * 2)
    stop_event = threading.Event()
    workers = [threading.Thread(target=prefetch_worker, args=(camera_queue, prefetch_queue, stop_event), daemon=True) 
    ...
    try:
        for iteration in range(first_iter, opt.iterations + 1):
            ...
            # Pick a random Camera and prefetch next ones
            if not viewpoint_stack:
                viewpoint_stack = scene.getTrainCameras().copy()
            
            # Get prefetched camera or fallback to direct loading
            if not prefetch_queue.empty():
                viewpoint_cam = prefetch_queue.get()
            else:
                viewpoint_cam = viewpoint_stack.pop(randint(0, len(viewpoint_stack)-1))
                viewpoint_cam.original_image = viewpoint_cam.original_image.cuda()
                viewpoint_cam.semantic_feature = viewpoint_cam.semantic_feature.cuda()
            
            # Prefetch next cameras
            while len(viewpoint_stack) > 0 and not camera_queue.full():
                next_cam = viewpoint_stack.pop(randint(0, len(viewpoint_stack)-1))
                camera_queue.put(next_cam)
            ...
            image = F.interpolate(image.unsqueeze(0), size=(gt_image.shape[1], gt_image.shape[2]), mode='bilinear', align_corners=True).squeeze(0)
            ...
            with torch.no_grad():
                ...
                if iteration in testing_iterations or iteration % 1000 == 0:
                    training_report(tb_writer, iteration, Ll1, Ll1_feature, loss, l1_loss, 0, testing_iterations, scene, render, (pipe, background))
        ...
        stop_event.set()
        # Clear queues and signal workers to stop
        ...
        for _ in range(num_workers):
            try:
                camera_queue.put(None, timeout=0.1)
            except:
                pass
        for w in workers:
            w.join(timeout=1.0)

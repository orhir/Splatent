...
def render_set(out_path, views, gaussians, pipeline, background, is_train):
    # Create output directories based on is_train flag
    if is_train:
        render_dir = os.path.join(out_path, 'render', "train")
        gt_dir = os.path.join(out_path, 'render', "train_gt")
    else:
        render_dir = os.path.join(out_path, 'render', "nvs")
        gt_dir = os.path.join(out_path, 'render', "nvs_gt")
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        torch.save(feature_map.detach().cpu(), os.path.join(render_dir, f"{img_name}.pt"))
        torch.save(gt_feature_map.detach().cpu(), os.path.join(gt_dir, f"{img_name}.pt"))
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool): 
    with torch.no_grad():
        gaussians = GaussianModel(dataset.sh_degree, feature_sh_degree=pipeline.feature_sh_degree)
        ...
        if not skip_train:
             render_set(dataset.model_path, scene.getTrainCameras(), gaussians, pipeline, background, is_train=True)

        if not skip_test:
             render_set(dataset.model_path, scene.getTestCameras(), gaussians, pipeline, background, is_train=False)
...
if __name__ == "__main__":
    ...
    if not hasattr(args, 'eval') or args.eval is None:
        args.eval = True
